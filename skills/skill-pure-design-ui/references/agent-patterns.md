...
| Autonomy levels | `.seg` with one stop per posture (observe, suggest, ask first, act and report) and one line stating the current consequence | `--radius-control` | `--accent` on the selected stop only | `role="radiogroup"` with arrow keys; the consequence line is the one supporting text allowed here, because a wrong posture sends real actions |
